...
describe('Starting tests for angular-app', function() {
  ...
  it('network-name should be ip-right-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('ip-right-network@0.0.1.bna');
    });
  });
  ...
  it('navbar-brand should be angular-app',() => {
    ...
    it('IpRight component should be loadable',() => {
      page.navigateTo('/IpRight');
      browser.findElement(by.id('assetName'))
      .then((assetName) => {
        return assetName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('IpRight');
      });
    });

    it('IpRight table should have 7 columns',() => {
      page.navigateTo('/IpRight');
